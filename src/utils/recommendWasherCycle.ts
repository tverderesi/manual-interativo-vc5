...
import { fuzzyMatchAny, hasAnyKeyword } from './textMatch';
...
function matchesKeywords(description: string, keywords: string[], threshold = 0.84): boolean {
  return hasAnyKeyword(description, keywords) || fuzzyMatchAny(description, keywords, threshold);
}
...
export function recommendWasherCycle(input: WasherInput): WasherRecommendation {
  ...
  if (matchesKeywords(d, flammable)) {
  ...
  if (matchesKeywords(d, ['la', 'wool', 'trico', 'tricot'])) {
  ...
  } else if (matchesKeywords(d, ['lingerie', 'renda', 'delicado', 'transparente', 'vestido delicado'])) {
  ...
  } else if (matchesKeywords(d, ['bebe', 'roupa de bebe', 'mancha de comida'])) {
  ...
  } else if (input.allergySensitive || matchesKeywords(d, ['alergia', 'acaro', 'sensivel'])) {
  ...
  } else if (matchesKeywords(d, ['poliester', 'acrilico', 'poliamida', 'sintetico', 'dry fit', 'academia', 'gym'])) {
  ...
  } else if (matchesKeywords(d, ['misto', 'misturada', 'varias roupas', 'roupas variadas'])) {
  ...
  } else if (matchesKeywords(d, ['algodao', 'camiseta', 'toalha', 'lencol', 'roupa do dia a dia'])) {
  ...
  if (matchesKeywords(d, ['muito suja', 'encardida', 'manchada'])) extraOptions.push('Pré-lavagem');

  if (input.wantsDrying === 'sim' || input.wantsDrying === 'talvez') {
    const cannotDry = matchesKeywords(d, ['la', 'borracha', 'plastico', 'tapete de banho', 'fibra de vidro']) || matchesKeywords(d, flammable);
    ...
    if (matchesKeywords(d, ['malha', 'trico', 'cobertor', 'edredom', 'bebe', 'vestido de festa'])) warnings.push('Cuidado especial na secagem: malhas, tricô, cobertores, edredons, roupas de bebê e vestidos de festa exigem atenção à etiqueta.');

    if (cannotDry) {
      rec.drying = { recommended: false, reason: 'Descrição contém itens não recomendados para secagem em máquina.' };
    } else {
      let mode = 'Normal';
      if (matchesKeywords(d, ['passar', 'ferro'])) mode = 'Ferro';
      else if (matchesKeywords(d, ['delicado'])) mode = 'Baixa temperatura';
      else if (matchesKeywords(d, ['pesado', 'toalhao'])) mode = 'Normal Eco';
      else if (matchesKeywords(d, ['algodao', 'toalha']) && input.loadSize === 'minima') mode = 'Tempo 30 minutos';
      else if (matchesKeywords(d, ['algodao', 'toalha']) && input.loadSize === 'pequena') mode = 'Tempo 60 minutos';
      else if (matchesKeywords(d, ['algodao', 'toalha']) && input.loadSize === 'media') mode = 'Tempo 120 minutos';
